...
int lua_skyla_spine_new(lua_State* L)
{
    int n = lua_gettop(L);
    ...
    if (n == 2) {
        ...
        luaL_error(L, "invalid number of arguments for skyla.spine.new, expected 2, got %d", n);
    }
    return 0;
}

int lua_skyla_spine_set_anim(lua_State* L)
{
    int n = lua_gettop(L);
    if (n == 4) {
...
int lua_skyla_spine_get_all_anim_names(lua_State* L)
{
    int n = lua_gettop(L);
    if (n == 1) {
        lua_getfield(L, 1, "__cobj");
...
int luaopen_skyla_spine(lua_State* L)
{
    ...
    luaL_Reg lib[] = {
        { "new",  lua_skyla_spine_new },
        { "set_anim", lua_skyla_spine_set_anim },
        { "get_all_anim_names", lua_skyla_spine_get_all_anim_names },
        { NULL, NULL },
    };
